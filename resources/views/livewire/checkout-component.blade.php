<main id="main" class="main-site">

    <div class="container">

        <div class="wrap-breadcrumb">
            <ul>
                <li class="item-link"><a href="/" class="link">home</a></li>
                <li class="item-link"><span>checkout</span></li>
            </ul>
        </div>
        <div class=" main-content-area">
            <form wire:submit.prevent="placeorder">
            <div class="row">
                <div class="col-md-12">
                    <div class="wrap-address-billing">
                        <h3 class="box-title">Billing Address</h3>
                        <div class="billing-address">
                            <p class="row-in-form">
                                <label for="fname">last name<span>*</span></label>
                                <input type="text" namefirst name="fname" value="" placeholder="Your first name" wire:model="firstName">
                            </p>
                            @error('firstName')
                            <p class="text-danger">{{ $message }}</p>
                            @enderror
                            <p class="row-in-form">
                                <label for="lname">last name<span>*</span></label>
                                <input type="text" name="lname" value="" placeholder="Your last name" wire:model="lastName" >
                            </p>
                            @error('lastName')
                            <p class="text-danger">{{ $message }}</p>
                            @enderror
                            <p class="row-in-form">
                                <label for="email">Email Addreess:</label>
                                <input type="email" name="email" value="" placeholder="Type your email" wire:model="email">
                            </p>
                            @error('email')
                            <p class="text-danger">{{ $message }}</p>
                            @enderror
                            <p class="row-in-form">
                                <label for="phone">Phone number<span>*</span></label>
                                <input type="number" name="phone" value="" placeholder="10 digits format" wire:model="mobile">
                            </p>
                            @error('mobile')
                            <p class="text-danger">{{ $message }}</p>
                            @enderror
                            <p class="row-in-form">
                                <label for="add">line1:</label>
                                <input type="text" name="add" value="" placeholder="Street at apartment number" wire:model="line1">
                            </p>
                            @error('line1')
                            <p class="text-danger">{{ $message }}</p>
                            @enderror
                            <p class="row-in-form">
                                <label for="add">line2:</label>
                                <input type="text" name="add" value="" placeholder="Street at apartment number" wire:model="line2">
                            </p>
                            @error('line2')
                            <p class="text-danger">{{ $message }}</p>
                            @enderror
                            <p class="row-in-form">
                                <label for="add">city:</label>
                                <input type="text" name="add" value="" placeholder="Street at apartment number" wire:model="city">
                            </p>
                            @error('city')
                            <p class="text-danger">{{ $message }}</p>
                            @enderror
                            <p class="row-in-form">
                                <label for="add">provine:</label>
                                <input type="text" name="add" value="" placeholder="Street at apartment number" wire:model="province">
                            </p>
                            @error('province')
                            <p class="text-danger">{{ $message }}</p>
                            @enderror
                            <p class="row-in-form">
                                <label for="country">Country<span>*</span></label>
                                <input type="text" name="country" value="" placeholder="United States" wire:model="country">
                            </p>
                            @error('country')
                            <p class="text-danger">{{ $message }}</p>
                            @enderror
                            <p class="row-in-form">
                                <label for="zip-code">Postcode / ZIP:</label>
                                <input type="number" name="zip-code" value="" placeholder="Your postal code" wire:model="zipCode">
                            </p>
                            @error('zipeCode')
                            <p class="text-danger">{{ $message }}</p>
                            @enderror

                            <p class="row-in-form fill-wife">
                                <label class="checkbox-field">
                                    <input name="different-add" id="different-add" value="forever" type="checkbox" wire:model="ship_to_diffrent">
                                    <span>Ship to a different address?</span>
                                </label>
                            </p>
                        </div>
                    </div>
                </div>
                @if($ship_to_diffrent)
                <div class="col-md-12">
                    <div class="wrap-address-billing">
                        <h3 class="box-title">Shipping Address</h3>
                        <div class="billing-address">
                            <p class="row-in-form">
                                <label for="fname">last name<span>*</span></label>
                                <input type="text" namefirst name="fname" value="" placeholder="Your first name" wire:model="s_firstName">
                            </p>
                            @error('s_firstName')
                            <p class="text-danger">{{ $message }}</p>
                            @enderror
                            <p class="row-in-form">
                                <label for="lname">last name<span>*</span></label>
                                <input type="text" name="lname" value="" placeholder="Your last name" wire:model="s_lastName" >
                            </p>
                            @error('s_lastName')
                            <p class="text-danger">{{ $message }}</p>
                            @enderror
                            <p class="row-in-form">
                                <label for="email">Email Addreess:</label>
                                <input type="email" name="email" value="" placeholder="Type your email" wire:model="s_email">
                            </p>
                            @error('s_email')
                            <p class="text-danger">{{ $message }}</p>
                            @enderror
                            <p class="row-in-form">
                                <label for="phone">Phone number<span>*</span></label>
                                <input type="number" name="phone" value="" placeholder="10 digits format" wire:model="s_mobile">
                            </p>
                            @error('s_mobile')
                            <p class="text-danger">{{ $message }}</p>
                            @enderror
                            <p class="row-in-form">
                                <label for="add">line1:</label>
                                <input type="text" name="add" value="" placeholder="Street at apartment number" wire:model="s_line1">
                            </p>
                            @error('s_line1')
                            <p class="text-danger">{{ $message }}</p>
                            @enderror
                            <p class="row-in-form">
                                <label for="add">line2:</label>
                                <input type="text" name="add" value="" placeholder="Street at apartment number" wire:model="s_line2">
                            </p>
                            @error('s_line2')
                            <p class="text-danger">{{ $message }}</p>
                            @enderror
                            <p class="row-in-form">
                                <label for="add">city:</label>
                                <input type="text" name="add" value="" placeholder="Street at apartment number" wire:model="s_city">
                            </p>
                            @error('s_city')
                            <p class="text-danger">{{ $message }}</p>
                            @enderror
                            <p class="row-in-form">
                                <label for="add">provine:</label>
                                <input type="text" name="add" value="" placeholder="Street at apartment number" wire:model="s_province">
                            </p>
                            @error('s_province')
                            <p class="text-danger">{{ $message }}</p>
                            @enderror
                            <p class="row-in-form">
                                <label for="country">Country<span>*</span></label>
                                <input type="text" name="country" value="" placeholder="United States" wire:model="s_country">
                            </p>
                            @error('s_country')
                            <p class="text-danger">{{ $message }}</p>
                            @enderror
                            <p class="row-in-form">
                                <label for="zip-code">Postcode / ZIP:</label>
                                <input type="number" name="zip-code" value="" placeholder="Your postal code" wire:model="s_zipCode">
                            </p>
                            @error('s_zipCode')
                            <p class="text-danger">{{ $message }}</p>
                            @enderror

                        </div>
                    </div>
                </div>
                @endif
            </div>

            <div class="summary summary-checkout">
                <div class="summary-item payment-method">
                    <h4 class="title-box">Payment Method</h4>
                    <p class="summary-info"><span class="title">Check / Money order</span></p>
                    <p class="summary-info"><span class="title">Credit Cart (saved)</span></p>
                    <div class="choose-payment-methods">
                        <label class="payment-method">
                            <input name="payment-method" id="payment-method-bank" value="cod" type="radio" wire:model="paymentMode">
                            <span>Cash On Delivery</span>
                            <span class="payment-desc">order now pay on delivery</span>
                        </label>
                        <label class="payment-method">
                            <input name="payment-method" id="payment-method-visa" value="card" type="radio" wire:model="paymentMode">
                            <span>Debit / Credit card payment</span>
                            <span class="payment-desc">There are many variations of passages of Lorem Ipsum available</span>
                        </label>
                        <label class="payment-method">
                            <input name="payment-method" id="payment-method-paypal" value="paypal" type="radio" wire:model="paymentMode">
                            <span>Paypal</span>
                            <span class="payment-desc">You can pay with your credit</span>
                            <span class="payment-desc">card if you don't have a paypal account</span>
                        </label>
                    </div>
                    @if(Session::has('checkout'))
                    <p class="summary-info grand-total"><span>Grand Total</span> <span class="grand-total-price">${{ Session::get('checkout')['total'] }}</span></p>
                    @endif
                    <button type="submit" class="btn btn-medium">Place order now</button>
                </div>
                <div class="summary-item shipping-method">
                    <h4 class="title-box f-title">Shipping method</h4>
                    <p class="summary-info"><span class="title">Flat Rate</span></p>
                    <p class="summary-info"><span class="title">Fixed $0</span></p>
                </div>
            </div>

        </form>

        </div><!--end main content area-->
    </div><!--end container-->

</main>